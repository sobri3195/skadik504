...
export default function Home() {
  ...
  return <div className='stack' id='home'>
    ...
    <section className='grid cols-4'>{[['25', 'Program Pendidikan'], ['8', 'Materi FASH'], ['6', 'Video Praktik'], ['5', 'Tahap Simulasi BLS']].map(([n, t]) => <article className='card' key={t}><h3>{n}</h3><p>{t}</p></article>)}</section>
    ...
    <section id='program'><h2>Program Pendidikan</h2><div className='grid cols-2'><article className='card'><h3>Kejuruan</h3><p>16 Program</p><Link className='btn small' to='/pendidikan'>Lihat Pendidikan</Link></article><article className='card'><h3>Dikkualsus</h3><p>9 Program</p><Link className='btn small' to='/pendidikan'>Lihat Pendidikan</Link></article></div></section>
